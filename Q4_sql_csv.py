...
tripdata_df = my_spark_session.read.option("header", True).schema(yellow_tripdata_schema).csv(input_file_path_2024).select(col("tpep_pickup_datetime"), col("VendorID"))
...
night_trips_df.coalesce(1).write.mode("overwrite").option("header", True).csv(f"{output_dir_path}/Q4_sql_csvInput_output")
