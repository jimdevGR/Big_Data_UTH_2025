"""
Το spark-defaults.conf είναι ρυθμισμένο με 2 executors, 4 cores και 8GB μνήμης ram,
επομένως αυτή η υλοποίηση εφαρμόζει κάθετη κλιμάκωση.
"""
...
my_spark_session = SparkSession \
    .builder \
    .appName("Q6_df_vertical_scaling") \
    .getOrCreate() \
...
finances_df.coalesce(1).write.mode("overwrite").option("header", True).csv(f"{output_dir_path}/Q6_df_vertical_scaling_output")
